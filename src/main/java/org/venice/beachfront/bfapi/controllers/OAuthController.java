...
public class OAuthController {
	...
	public String oauthStartExchange(HttpServletResponse response) {
		...
		String redirectUri = UriComponentsBuilder.fromHttpUrl(this.oauthAuthorizeUrl)
			.queryParam("client_id", this.oauthClientId)
			.queryParam("redirect_uri", callbackRedirectUri)
			.queryParam("response_type", "code")
			.queryParam("scope", "UserProfile.me")
	...
	@ResponseBody
	public String oauthCallback(@RequestParam("code") String authCode, HttpSession session, HttpServletResponse response) throws UserException {
		...
		String uiRedirectUri = UriComponentsBuilder.newInstance()
				.scheme("https")
				.host(this.domain)
				.queryParam("logged_in", "true")
				.build().toUri().toString();
	...
	public String oauthLogout(HttpSession session, HttpServletResponse response) throws UserException {
		...
		String logoutRedirectUri = UriComponentsBuilder.newInstance()
				.scheme("https")
				.host(oauthLogoutUrl)
				.queryParam("end_url", uiUrl)
				.build().toUri().toString();
