...
public class ProfileResponseBody {
	...
	public String getComputedUserId() throws UserException {
		...
		if (this.commonName != null && this.commonName.length() > 0 &&
				this.memberOf != null && this.memberOf.length() > 0) {
			return String.format("%s@%s", this.commonName, this.memberOf);
		}
		throw new UserException("Could not obtain a user ID from OAuth profile response", this.toString(), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public String getComputedUserName() throws UserException {
		...
		throw new UserException("Could not obtain a user name from OAuth profile response", this.commonName, HttpStatus.INTERNAL_SERVER_ERROR);
	}
